...
$(document).ready(function () {
	...
	jQuery.validator.addMethod("katakana", function(value, element) {
		return this.optional(element) || value.match(/^[\u30A1-\u30FA]+$/);
		}, "全角カタカナで入力して下さい。"
	);

	// furigana
    jQuery.validator.addMethod("furigana", function(value, element) {
            return this.optional(element) || value.match(/^[a-zA-Z0-9ａ-ｚ０-９ｧ-ﾝﾞﾟァ-ヶーぁ-んァ-ヶーぁ-ん。 s]+$/);
        }, "漢字が入力できません。"
    );
	...
	$.validator.addMethod("alphanumericspecial", function(value, element) {
        ...
        return this.optional(element) || /^[a-zA-Z0-9]+$/i.test(value);
    }, 'allow alphabet, number and some special character');
	...
	// validate extension (default is photo)
	$.validator.addMethod( "extension", function( value, element, param ) {
        param = typeof param === "string" ? param.replace( /,/g, "|" ) : "jpeg|png|gif|jpg";
...
// errorClass = ".error" || errorClass = ".error, .custom_error"
function scrollToFirstErrorElement(errorClass = '.error, .custom_error'){ 
    $(errorClass).each(function () {
        let that = this;
        if($.trim($(that).text()) != '') {
            $('html, body').animate({
                scrollTop: $(that).offset().top - 300
            }, 500);
            return false;
        }
    });
